...
class DiaryreportController extends Controller
{
	...
	public function actionView($id)
	{
		...
		for ($i=0;$i<$cnt;$i++)
		{
			$_more_than_3_days[$i]['comment']="<font color=red>" .(($_comments=Comments::model()->findByAttributes(array('document_id'=>$_more_than_3_days[$i]['id']))) ? $_comments->comment : '') . "</font><BR>" .
		...
		for ($i=0;$i<$cnt;$i++)
		{
			$_more_than_5_days[$i]['comment']="<font color=red>" .(($_comments=Comments::model()->findByAttributes(array('document_id'=>$_more_than_5_days[$i]['id']))) ? $_comments->comment : '') . "</font><BR>" .
		...
		for ($i=0;$i<$cnt;$i++)
		{
			$_more_than_7_days[$i]['comment']="<font color=red>" .(($_comments=Comments::model()->findByAttributes(array('document_id'=>$_more_than_7_days[$i]['id']))) ? $_comments->comment : '') . "</font><BR>" .
		...
		$this->render('view', array(
			...
			'officerTitle'=>Officers::model()->getOfficerTitle($id),
		));		
	}
...
//		print_r($this->_report);
	...
	public function actionPview($id)
	{
		...
		{	
			...
			for ($i=0;$i<$cnt;$i++)
			{
				$_more_than_3_days[$i]['comment']="<font color=red>" .(($_comments=Comments::model()->findByAttributes(array('document_id'=>$_more_than_3_days[$i]['id']))) ? $_comments->comment : '') . "</font>" .
		...
		if ($_more_than_10_days)
		{
			...
			for ($i=0;$i<$cnt;$i++)
			{
				$_more_than_10_days[$i]['comment']="<font color=red>" .(($_comments=Comments::model()->findByAttributes(array('document_id'=>$_more_than_10_days[$i]['id']))) ? $_comments->comment : '') . "</font><BR>" .
